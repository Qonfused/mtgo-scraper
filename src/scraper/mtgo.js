// eslint globals for no-undef rule
/* global document */
...
async function scrapeEvent(page, { url, uri, format, type, date, eventID }) {
  ...
  const decks = Object.fromEntries(
    await page.evaluate(() =>
      Array.from(
        document.querySelectorAll('section.decklist header:nth-child(1) a:nth-child(1)')
      ).map(e => {
        const username = decodeURI(e.href.match(/(?<=#deck_).*/)?.[0]);
        const cards = document
          .querySelector(
            `section[id="deck_${username}"] div.decklist-cards-and-preview div:nth-child(1)`
          )
          .textContent.split('\n')
  ...
  const bracket = (
    await page.evaluate(() =>
      Array.from(document.querySelectorAll('div.decklist-bracket-player')).map(e =>
        e?.textContent.trim()?.split(/[\s]{2}/)
      )
    )
  ).reduce((acc, e, i) => {
  ...
  const top8 = !bracket.length
    ? {}
    : {
        [bracket[0]['player1']]: '1st',
        [bracket[0]['player2']]: '8th',
        [bracket[1]['player1']]: '4th',
        [bracket[1]['player2']]: '5th',
        [bracket[2]['player1']]: '2nd',
        [bracket[2]['player2']]: '7th',
        [bracket[3]['player1']]: '3rd',
        [bracket[3]['player2']]: '6th',
      };
    ...
    (
      await page.evaluate(() => {
        const rows = Array.from(
          document.querySelectorAll('table.decklist-standings tbody tr')
        )
          ?.map(e =>
            Array.from(e.querySelectorAll('td')).map(elem => {
      ...
      username,
      { record, rank: top8?.[username] || r, points, OMWP, GWP, OGWP },
    ])
  );

  const players = Object.keys(decks).map(username => ({
    username,
