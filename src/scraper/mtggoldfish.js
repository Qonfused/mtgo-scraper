...
export const scrapeGoldfish = async (page, { uri, date }) => {
  ...
  const archetypes = await page.evaluate(() =>
    Array.from(
      document.querySelectorAll(
        'div.deck-display-right-contents table.table:nth-child(2) tr td a[href]'
      )
    )
  ...
  const players = await page.evaluate(() =>
    Array.from(
      document.querySelectorAll(
        'div.deck-display-left-contents table.table-tournament tr td a[href]'
      )
    )
      .reduce((a, b, i, array) => {
  ...
  while (!data.length || data.filter(({ archetype }) => !archetype).length) {
    ...
    data = await page.evaluate(() =>
      Array.from(document.querySelectorAll('tr.tournament-decklist')).map(e => ({
        id: parseInt(e.getAttribute('data-deckid')),
        archetype: e
          .querySelector('div.header-left h1.title')
          ?.textContent?.match(/(?<=\n).*/)?.[0],
        displayName: e
          .querySelector('.deck-container-information')
          ?.textContent?.match(/(?<=\nArchetype:\s+).*/)?.[0],
      }))
    );
  }
  ...
  return Object.fromEntries(
    players.map(({ player, _id }) => {
      const {
        ...
        archetype,
        displayName,
      } = data.filter(({ id }) => id === _id)[0];
      const archetype_uid = archetypes.filter(
        ({ _displayName }) => _displayName === displayName
      )[0]?.uid;
      return [
        player,
        {
